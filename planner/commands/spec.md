...
for (const ac of ac_items) {
  // 주관적/모호한 표현 탐지
  subjective_patterns = [
    /편하게|쉽게|빠르게|자연스럽|직관적|충분히|원활하게|신속하게|깔끔하게|간편하게|효율적|효과적|안정적/,  // 주관적 부사/형용사 (어간 기준)
    /개선|향상|최적화/,                       // 측정 불가 동사 (수치 없이 단독 사용 시)
    /적절|합리적|좋[은다]|나[은다]|깨끗/,     // 주관적 형용사 (어간 매칭)
    /등등|기타|여러|다양한|많은|적은/          // 범위 모호
  ]
