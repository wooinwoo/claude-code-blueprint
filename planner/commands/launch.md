...
AskUserQuestion([
  {
    ...
    header: "변경 범위",
    options: [
      { label: "Jira Epic 기준", description: "Epic 하위 완료 이슈로 생성" },
      { label: "날짜 범위 지정", description: "특정 기간 내 완료 이슈" },
...
// ⚠️ PROJ → 실제 Jira 프로젝트 키로 대체 (CLAUDE.md 또는 사용자 입력에서 확인)
try {
  const done_issues = mcp__jira__jira_search({
  ...
  console.log("⚠️ Jira 접근 불가. 수동 입력 모드로 전환합니다.")
}
```

#### 1-3. 수동 입력 폴백 (Jira 없을 때)

```typescript
// Jira 미연결 시 사용자에게 직접 변경사항 입력 요청
AskUserQuestion([
  {
    question: "이번 릴리스에 포함된 주요 변경사항을 알려주세요",
    header: "변경사항 입력",
    description: "카테고리별로 구분해서 입력하면 더 좋은 릴리스 노트가 생성됩니다.\n예: 신규 기능: OO 추가 / 버그 수정: OO 오류 해결 / 개선: OO 성능 향상"
  }
])
```
...
/launch 상품-검색 --notes
```
→ Jira 완료 이슈 수집 → 사용자용/내부용 릴리스 노트 생성 → plans/release-notes-상품-검색.md 저장
