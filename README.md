...
|---------|------|--------|----------|-------|-----|
| **react-next** | React/Next.js 개발자 | 16 | 15 | 21 | 9 |
| **nestjs** | NestJS 백엔드 개발자 | 16 | 13 | 18 | 9 |
| **fullstack** | 프론트 + 백엔드 풀스택 | 19 | 15 | 23 | 9 |
| **designer** | UI/UX + 퍼블리싱 | 3 | 9 | 8 | 6 |
| **planner** | PM / 기획자 | 3 | 14 | 7 | 4 |

> 커맨드별 상세 설명: [docs/commands.md](docs/commands.md)

### Dev (react-next, nestjs, fullstack)

| Command | 설명 |
|---------|------|
| `/orchestrate` | 6-Phase 파이프라인: 설계 → 브랜치 → 구현 → 리뷰 → PR → 정리. `--full`로 고품질 |
| `/code-review` | 5개 리뷰 에이전트 병렬 실행 (코드, 컨벤션, 보안, 성능, 타당성) |
| `/build-fix` | lint → type → build 에러 자동 수정 루프 |
| `/test-coverage` | 커버리지 분석 + 미커버 테스트 자동 생성 |
| `/lighthouse` | 페이지별 Lighthouse 성능/접근성/SEO 분석 |
| `/refactor-clean` | knip 기반 데드코드 분석 + 자동 제거 |
