...
```bash
# 기본: 현재 브랜치 vs base 브랜치 (main/dev)
BASE=$(git merge-base HEAD main 2>/dev/null || git merge-base HEAD dev 2>/dev/null || echo "HEAD~1")
git diff $BASE..HEAD
...
```
변경 파일 분류:
  Components:  src/features/bid/BidList.tsx, src/features/bid/BidCard.tsx
  Hooks:       src/hooks/useBidSearch.ts
  Utils:       src/lib/format.ts
  Styles:      src/features/bid/bid.css
  Tests:       src/features/bid/__tests__/BidList.test.tsx
  Config:      -
```
