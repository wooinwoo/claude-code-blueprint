...
# 실패 시 (스크립트 없음, git 에러 등): 수동으로 git diff 실행 후 파싱
```

> **스크립트 실패 폴백**: collect-diff.js가 없거나 에러나면 `git diff --stat main...HEAD` + `git diff main...HEAD`를 직접 실행하여 수동 파싱.
...
여러 에이전트가 같은 파일/라인을 지적하면 통합:
- 같은 파일:라인 → 가장 높은 심각도로 통합, 관련 에이전트 이름 병기
- 예: code-reviewer "함수 65줄" + react-reviewer "useCallback 누락" → 통합 이슈로 묶기
- 모순되는 제안 (보안 "암호화 추가" vs 성능 "오버헤드 제거") → 보안 우선
